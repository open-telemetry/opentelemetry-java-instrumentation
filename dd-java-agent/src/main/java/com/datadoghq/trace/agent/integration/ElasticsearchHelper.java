...
/**
 * Instrument all Elasticsearch queries. We have not found a way to inject the opentracing
 * contribution, so this integration is the integration. FIXME find a better to way to inject the OT
 * contrib
 */
public class ElasticsearchHelper extends DDAgentTracingHelper<ActionListener> {
  ...
  /**
   * Strategy: When a query is executed, if start the integration and a new Span. We override the
   * default FutureAction by using the one provided in the opentracing contribution.
   *
