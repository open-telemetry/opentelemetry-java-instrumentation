...
public class TraceAnnotationsManager {
  ...
  private static final String EXCEPTION_EXIT_RULE =
      "BIND span:"
          + ActiveSpan.class.getName()
          + " = getTracer().activeSpan()\n"
          + "IF span != null\n"
          + "DO\n"
          + "span.setTag("
          + Tags.class.getName()
  ...
  public TraceAnnotationsManager(final Retransformer trans, final TracingAgentConfig config) {
    transformer = trans;
  ...
  // FIXME: this needs to be done for each classloader.
