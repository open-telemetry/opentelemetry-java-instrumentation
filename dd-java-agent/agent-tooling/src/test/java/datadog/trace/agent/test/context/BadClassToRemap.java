...
public class BadClassToRemap {
  /**
   *  Callable does not have context class of type State.
   *  This should throw an exception during class transform.
   */
  public static int unmappedObjectError(Callable callable) {
    final State state = InstrumentationContext.get(callable, State.class);
    return ++state.anInt;
  }
