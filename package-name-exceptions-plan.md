...
| --- | --- | --- |
| Module-wide skips | `jmx-metrics` has 62 Java files | Do not start here; this needs a broader naming decision. |
...
Batch sizing: keep submitting four PRs at a time, but each PR can include up to about 40 changed files when the modules are closely related or the reference spread is easy to audit. Count dependent import updates and test package moves in that limit. Avoid combining package moves that require broad cross-module rewrites just to fill the file budget.
...
- `jmx-metrics`: current packages are under `jmx`, while the module says `jmx-metrics`. This touches 62 Java files and may be user-facing enough to deserve a dedicated PR.
...
- Remaining unversioned module allowlist entries split into policy buckets:
  ...
  - `*-common` modules need case-by-case module naming review:
    ...
    - `spring-webmvc-common` was completed in PR 25. Its PR 28 testing package follow-up and the PR 26 `spring-cloud-gateway-common` testing package work were completed together in #18839. `jetty-common`, `tomcat-common`, and `opensearch-rest-common` were completed in PRs 23-24.
