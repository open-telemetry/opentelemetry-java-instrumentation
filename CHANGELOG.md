...
- Deprecate the Java agent and Spring Boot starter's bundled contrib samplers, including the
  ...
  Both names continue to work but log a warning when selected. For rule-based routing, consider
  the SDK incubator `composite/development` `rule_based` sampler; there is no links-based replacement.
