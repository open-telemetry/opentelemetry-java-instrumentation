...
- Runtime telemetry now emits `package.checksum_algorithm=SHA-256` and 64-character
  `package.checksum` values instead of SHA1 checksums.
  ([#18846](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18846))
- Lettuce Redis span names no longer include `db.namespace`.
  ([#18984](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18984))

### 🚫 Deprecations

- Renames the common logging context keys to `otel.instrumentation.common.logging.trace-id-key`,
...
- Adds `otel.semconv-stability.v3-preview` as a fallback for the declarative `v3_preview` semconv
  stability setting.
  ([#18936](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18936))
- The `kafka-broker` and `kafka-connect` JMX target systems are renamed to
  `experimental-kafka-broker` and `experimental-kafka-connect`; the old names still work for now but
  log a warning.
...
- Add Java agent instrumentation for HBase client 2.0.0 through 2.5.0.
  ([#18253](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18253))
- Adds `opentelemetry-api-1.63` Javaagent instrumentation for OpenTelemetry API 1.63 and
  `opentelemetry-api-incubator` 1.63.
  ([#18911](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18911))

### 📈 Enhancements

- Add opt-in `otel.semconv.exception.signal.preview=logs` support for emitting HTTP request
  exceptions as log signals.
...
- Jedis instrumentation now emits stable database semantic-convention attributes, including
  `db.namespace` and server address/port, on Jedis spans.
...
- Add `db.collection.name` to DynamoDB spans when stable database semantic conventions are enabled.
  ([#18853](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18853))
- Set the instrumentation version on emitted Meter scopes.
...
- RPC exceptions are emitted as log records when `otel.semconv.exception.signal.preview=logs` is
  enabled.
...
- AWS SDK request failures now emit `rpc.client.call.exception` log records when
  `otel.semconv.exception.signal.preview=logs` is enabled.
  ([#18894](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18894))
- Semantic-convention selection now honors declarative per-domain `semconv.version`,
  `semconv.experimental`, and `semconv.dual_emit` settings.
  ([#18908](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18908))
- Moves the nonstable `service.peer` and `rpc` semconv opt-ins to
...
- Cassandra client spans no longer emit `network.type` when stable database semconv output is
  enabled.
  ([#18920](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18920))
- `SqlQueryAnalyzer.analyze()` now uses the summary-aware path when
...
- Cassandra spans now include stable `db.operation.name` and `db.collection.name` attributes.
  ([#18922](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18922))
- Couchbase 3.x spans now emit stable database semantic-convention attributes when
  `otel.semconv-stability.opt-in=database` is set, with experimental `db.couchbase.*` attributes
  behind `otel.instrumentation.couchbase.experimental-span-attributes=true`.
...
- Enable `jvm.gc.cause` on GC duration metrics when stable JVM semantic conventions are enabled.
  ([#18967](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18967))
- Adds R2DBC batch-size semconv attributes behind `otel.semconv-stability.opt-in=database`.
  ([#18980](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18980))
- Support Redisson batch operations by recording `db.operation.name` for pipeline and multi-command
  batches.
  ([#18985](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18985))
- Vert.x Redis client spans now include the Redis host and port in the span name when stable
  database semconv is enabled.
  ([#18986](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18986))
- Mongo DB client spans use the stable collection-only name format when
...
- Adds `db.collection.name` support to DB client instrumentation and SQL span names when stable
  database semconv is enabled.
  ([#18989](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18989))
- HBase spans now populate stable database semconv attributes with `db.name` as the table namespace
  and `db.collection_name` as the table name when stable database semconv is enabled.
  ([#18990](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18990))
- Geode spans now populate `db.collection.name` for stable database semconv while preserving legacy
  `db.name` behavior for the old semconv path.
  ([#18992](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18992))
- Couchbase spans now include the bucket name in the span name when
  `otel.semconv-stability.opt-in=database` is enabled.
  ([#18993](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18993))
- JDBC transaction operations now emit duration metrics for `COMMIT` and `ROLLBACK`.
  ([#19003](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19003))
- Vert.x SQL batch queries now set `db.operation.batch_size` on spans when stable database semconv
  is enabled.
  ([#19004](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19004))
- Add `db.operation.batch.size` to Redisson batch spans.
  ([#19005](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19005))
- Set the `okhttp-3.0` instrumentation version from the generated `InstrumentationVersion` class.
...
- Stops the Pulsar agent from creating duplicate receive spans for multi-topic consumers.
...
- Fix a `Bedrock` `converseStream` NPE when a tool call has empty arguments.
...
- Parse Amazon Aurora JDBC URLs with `jdbc:aws-dsql:` and `jdbc:aws-wrapper:` prefixes correctly.
  ([#18831](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18831))
- Annotated constructors are no longer matched as `@WithSpan`/`@AddingSpanAttributes` methods,
  avoiding constructor instrumentation errors and preserving method attributes.
  ([#18836](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18836))
- Keep Vert.x 5.1.x HTTP and SQL client instrumentation working after the upstream package changes,
  restoring peer address/port capture and query context propagation.
  ([#18858](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18858))
- Run `@WithSpan` instrumentation after other instrumentations so other advice can attach attributes
  to the active span.
  ([#18874](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18874))
- SQLite JDBC statements and prepared statements are now instrumented, restoring SQL spans and
  parameter capture for SQLite connections.
  ([#18885](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18885))
- Azure SDK spans now honor an explicitly supplied parent `Context` when creating child spans.
  ([#18886](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18886))
- Fix Logback MDC declarative config keys so `otel.instrumentation/development.java.*` settings are
  applied correctly.
...
- SQL query summaries now keep the original statement casing, so `select * from users` is summarized
  as `select users`.
...
- Restore log bridging for applications using `opentelemetry-api-incubator:1.47.0-alpha`.
  ([#18959](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18959))
...
- Fix DynamoDB stable database semconv for batch requests so `db.operation` and `db.collection.name`
  are populated correctly when `otel.semconv-stability.opt-in=database` is enabled.
  ([#18970](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18970))
- Fix DynamoDB batch spans to emit the correct `db.operation` and `db.operation.batch_size` for
  `BatchGetItem` and `BatchWriteItem`.
  ([#18979](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/18979))
- Fix Jedis instrumentation to match only methods, restoring command tracing for
  `redis.clients.jedis.Jedis`, `BinaryJedis`, and `UnifiedJedis`.
