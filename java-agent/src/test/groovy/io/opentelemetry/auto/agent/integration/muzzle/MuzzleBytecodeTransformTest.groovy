package io.opentelemetry.auto.agent.integration.muzzle
...
class MuzzleBytecodeTransformTest extends Specification {
  ...
  def "muzzle fields added to all instrumentation"() {
    ...
    for (Object instrumenter : ServiceLoader.load(IntegrationTestUtils.getAgentClassLoader().loadClass("io.opentelemetry.auto.agent.tooling.Instrumenter"), IntegrationTestUtils.getAgentClassLoader())) {
      ...
      if (!IntegrationTestUtils.getAgentClassLoader().loadClass('io.opentelemetry.auto.agent.tooling.Instrumenter$Default').isAssignableFrom(instrumenter.getClass())) {
