...
class TraceAssert {
  ...
  static void assertTrace(ListWriter writer, TraceId traceId, int expectedSize,
                          @ClosureParams(value = SimpleType, options = ['io.opentelemetry.auto.agent.test.asserts.TraceAssert'])
  ...
  void span(int index, @ClosureParams(value = SimpleType, options = ['io.opentelemetry.auto.agent.test.asserts.SpanAssert']) @DelegatesTo(value = SpanAssert, strategy = Closure.DELEGATE_FIRST) Closure spec) {
    if (index >= spans.size()) {
  ...
  void span(String name, @ClosureParams(value = SimpleType, options = ['io.opentelemetry.auto.agent.test.asserts.SpanAssert']) @DelegatesTo(value = SpanAssert, strategy = Closure.DELEGATE_FIRST) Closure spec) {
    int index = -1
