...
Within the subfolder, create three folders `core` (skip if manual instrumentation is not possible),
`auto`, and `testing`.
...
In addition, to prevent conflict between manual instrumentation loaded by the user and by the agent,
we make sure to create a shaded version with no dependencies for use from the auto instrumentation
at a separate package.
...
result of a library factory when delegating. The test should implement the `InstrumentationTestRunner`
trait for common setup logic. If the tests pass, manual instrumentation is working OK.
...
decorator will be identical to the one in the unit test you wrote above.
