...
public abstract class HttpServerTracer<RequestT, ResponseT, ConnectionT, StorageT>
    extends BaseTracer {
  ...
  public Context startSpan(RequestT request, ConnectionT connection, Method origin) {
    String spanName = spanNameForMethod(origin);
    return startSpan(request, connection, spanName);
  }

  public Context startSpan(RequestT request, ConnectionT connection, String spanName) {
    return startSpan(request, connection, spanName, -1);
  }

  public Context startSpan(
      RequestT request, ConnectionT connection, String spanName, long startTimestamp) {
   ...
   */
  public Scope startScope(Span span, StorageT storage) {
   ...
   * <p>Attaches new context to the request to avoid creating duplicate server spans.
   */
  public Scope startScope(Span span, StorageT storage, Context context) {
  ...
  // TODO should end methods remove SPAN attribute from request as well?
  public void end(Span span, ResponseT response, long timestamp) {
   ...
   */
  public void endExceptionally(Span span, Throwable throwable, ResponseT response) {
   ...
   */
  public void endExceptionally(Span span, Throwable throwable, ResponseT response, long timestamp) {
  ...
  public Span getServerSpan(StorageT storage) {
   ...
   */
  @Nullable
  public abstract Context getServerContext(StorageT storage);

  protected void onConnection(Span span, ConnectionT connection) {
  ...
  protected void onRequest(Span span, RequestT request) {
  ...
  protected void onConnectionAndRequest(Span span, ConnectionT connection, RequestT request) {
  ...
  private String clientIP(ConnectionT connection, RequestT request) {
    // try Forwarded
  ...
  @Nullable
  protected abstract Integer peerPort(ConnectionT connection);

  @Nullable
  protected abstract String peerHostIP(ConnectionT connection);

  protected abstract String flavor(ConnectionT connection, RequestT request);

  protected abstract TextMapPropagator.Getter<RequestT> getGetter();

  protected abstract String url(RequestT request);
  ...
  protected abstract int responseStatus(ResponseT response);

  /**
   * Stores given context in the given request-response-loop storage in implementation specific way.
   */
  protected abstract void attachServerContext(Context context, StorageT storage);
