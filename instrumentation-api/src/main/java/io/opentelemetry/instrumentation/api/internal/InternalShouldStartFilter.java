...
/**
 * Internal interface for filtering spans before they are started. This is the stable internal API
 * that corresponds to the incubator {@code ShouldStartFilter}.
 *
 * <p>This class is internal and is hence not for public use. Its APIs are unstable and can change
 * at any time.
 */
@FunctionalInterface
public interface InternalShouldStartFilter<REQUEST> {
   ...
   */
  static <REQUEST> InternalShouldStartFilter<REQUEST> none() {
    return (parentContext, request, spanKind, instrumentationName) -> true;
  }

  /** Combines multiple filters into a single composite filter. */
  static <REQUEST> InternalShouldStartFilter<REQUEST> allOf(
      List<InternalShouldStartFilter<REQUEST>> filters) {
    if (filters.isEmpty()) {
      return none();
    }
    if (filters.size() == 1) {
    ...
    List<InternalShouldStartFilter<REQUEST>> sortedFilters =
        filters.stream()
    ...
    return (parentContext, request, spanKind, instrumentationName) -> {
      for (InternalShouldStartFilter<REQUEST> filter : sortedFilters) {
