          ...
          .waitAndAssertLogRecords(
              log ->
                  log.hasSeverity(Severity.ERROR)
                      ...
                      .hasAttributesSatisfyingExactly(
                          equalTo(EXCEPTION_TYPE, status.getCause().getClass().getName()),
                          equalTo(EXCEPTION_MESSAGE, status.getCause().getMessage()),
          ...
          .waitAndAssertLogRecords(
              log ->
                  log.hasSeverity(Severity.ERROR)
                      .hasEventName("rpc.server.call.exception")
                      .hasAttributesSatisfyingExactly(
                          equalTo(EXCEPTION_TYPE, status.asRuntimeException().getClass().getName()),
                          equalTo(EXCEPTION_MESSAGE, status.asRuntimeException().getMessage()),
                          satisfies(EXCEPTION_STACKTRACE, val -> val.isNotNull())));
    }
