...
final class GrpcSpanStatusExtractor implements SpanStatusExtractor<GrpcRequest, Status> {
  @Override
  public StatusCode extract(GrpcRequest request, Status status, SpanKind kind, @Nullable Throwable error) {
    if (status == null) {
    ...
    return SpanStatusExtractor.getDefault().extract(request, status, kind, error);
  }
}
