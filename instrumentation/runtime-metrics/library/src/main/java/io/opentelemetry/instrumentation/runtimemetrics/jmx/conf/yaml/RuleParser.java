...
package io.opentelemetry.instrumentation.runtimemetrics.jmx.conf.yaml;
...
public class RuleParser {
  ...
  private static RuleParser theParser = null;

  public static RuleParser get() {
    if (theParser == null) {
      theParser = new RuleParser();
    }
    return theParser;
  }
   ...
   *
   * @param logger logger to be used for any messages
   * @param conf the metric configuration
   * @param is the InputStream with the YAML rules
   */
  public void addMetricDefs(Logger logger, MetricConfiguration conf, InputStream is) {
    try {
