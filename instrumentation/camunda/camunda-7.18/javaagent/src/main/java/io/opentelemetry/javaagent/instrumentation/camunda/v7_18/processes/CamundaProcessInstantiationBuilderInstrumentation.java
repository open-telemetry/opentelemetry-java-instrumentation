...
import io.opentelemetry.javaagent.instrumentation.camunda.v7_18.processes.CamundaActivityInstantiationBuilderSetter;
import io.opentelemetry.javaagent.instrumentation.camunda.v7_18.processes.CamundaProcessSingletons;
...
public class CamundaProcessInstantiationBuilderInstrumentation implements TypeInstrumentation {
	...
	public void transform(TypeTransformer transformer) {
		...
		transformer.applyAdviceToMethod(isMethod().and(named("executeWithVariablesInReturn")),
				this.getClass().getName() + "$CamundaProcessInstantiationBuilderExecuteWithVariablesAdvice");
	...
	public static class CamundaProcessInstantiationBuilderExecuteWithVariablesAdvice {
		...
		public static void addTracingEnter(
				@Advice.FieldValue("modificationBuilder") ProcessInstanceModificationBuilderImpl modificationBuilder,
			...
			if (CamundaProcessSingletons.getInstumenter().shouldStart(parentContext, request)) {
				...
				if (currentSpanContext.isValid()) {
					CamundaProcessSingletons.getOpentelemetry().getPropagators().getTextMapPropagator()
							.inject(context, modificationBuilder, new CamundaActivityInstantiationBuilderSetter());
				}
		...
		public static void closeTrace(@Advice.FieldValue("processDefinitionKey") String processDefinitionKey,
				@Advice.Local("request") CamundaCommonRequest request, @Advice.Local("otelContext") Context context,
