...
class JoddHttpHttpAttributeGetterTest {

  private static final JoddHttpHttpAttributeGetter attributeGetter =
      new JoddHttpHttpAttributeGetter();

  @Test
  void getMethod() {
    for (String method : Arrays.asList("GET", "PUT", "POST", "PATCH")) {
      assertEquals(method, attributeGetter.getHttpRequestMethod(new HttpRequest().method(method)));
    }
  }
  ...
  void getUrl() {
    ...
    assertEquals(
        ...
        attributeGetter.getUrlFull(request));
  }
  ...
  void getHttpRequestHeader() {
    ...
    List<String> headerVals = attributeGetter.getHttpRequestHeader(request, "single");
    assertEquals(1, headerVals.size());
    assertEquals("val1", headerVals.get(0));
    headerVals = attributeGetter.getHttpRequestHeader(request, "multiple");
    assertEquals(2, headerVals.size());
    assertEquals("val1", headerVals.get(0));
    assertEquals("val2", headerVals.get(1));
    headerVals = attributeGetter.getHttpRequestHeader(request, "not-existing");
  ...
  void getStatusCode() {
    for (Integer code :
      ...
      assertEquals(
          code,
          attributeGetter.getHttpResponseStatusCode(
  ...
  void getResponseHeader() {
    HttpResponse response =
        new HttpResponse()
            ...
            .header("multiple", "val1")
            .header("multiple", "val2");
    List<String> headerVals = attributeGetter.getHttpResponseHeader(null, response, "single");
    assertEquals(1, headerVals.size());
    assertEquals("val1", headerVals.get(0));
    headerVals = attributeGetter.getHttpResponseHeader(null, response, "multiple");
    assertEquals(2, headerVals.size());
    assertEquals("val1", headerVals.get(0));
    assertEquals("val2", headerVals.get(1));
    headerVals = attributeGetter.getHttpResponseHeader(null, response, "not-existing");
    assertEquals(0, headerVals.size());
  }
}
