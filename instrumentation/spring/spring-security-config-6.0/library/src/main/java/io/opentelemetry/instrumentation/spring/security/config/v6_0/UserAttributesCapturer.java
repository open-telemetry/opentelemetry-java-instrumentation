...
public class UserAttributesCapturer {
  ...
  /** Determines if {@code enduser.id}, or {@code user.name} in v3 preview, should be captured. */
   ...
   *       #setRoleGrantedAuthorityPrefix(String) role prefix}
   *   <li>{@code enduser.scope} - a comma-separated list from the {@link
   *       Authentication#getAuthorities()} with the configured {@link
   *       #setScopeGrantedAuthorityPrefix(String) scope prefix}
   * </ul>
   *
  ...
  public void captureUserAttributes(Context otelContext, @Nullable Authentication authentication) {
    if (authentication != null) {
      ...
      if (nameEnabled) {
        localRootSpan.setAttribute(v3Preview ? USER_NAME : ENDUSER_ID, authentication.getName());
      }
