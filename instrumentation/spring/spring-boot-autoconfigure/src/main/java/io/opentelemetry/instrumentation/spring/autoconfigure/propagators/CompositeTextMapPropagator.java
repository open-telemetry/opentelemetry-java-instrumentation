...
/** Composite of {@link TextMapPropagator}. Defaults to W3C and BAGGAGE. */
public final class CompositeTextMapPropagator implements TextMapPropagator {
  ...
  public Collection<String> fields() {
    return types.stream()
        .map(key -> mapping.getOrDefault(key, TextMapPropagator.noop()))
  ...
  public <C> void inject(Context context, @Nullable C c, TextMapSetter<C> textMapSetter) {
    types.stream()
        .map(key -> mapping.getOrDefault(key, TextMapPropagator.noop()))
        .forEach(p -> p.inject(context, c, textMapSetter));
  }
