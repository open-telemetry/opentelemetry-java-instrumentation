...
public final class NatsMessageWritableHeaders {
  ...
  public static Message create(String subject, String replyTo, byte[] body) {
    return NatsMessage.builder().subject(subject).replyTo(replyTo).headers(new Headers()).data(body).build();
  }
  ...
  public static Message create(String subject, String replyTo, Headers headers, byte[] body) {
    ...
    return NatsMessage.builder().subject(subject).replyTo(replyTo).headers(headers).data(body).build();
  }
  ...
  public static Message create(Message message) {
    if (message instanceof NatsMessage &&
      ...
      return NatsMessage.builder().subject(message.getSubject()).replyTo(message.getReplyTo())
          .headers(new Headers(message.getHeaders())).data(message.getData()).build();
    }
    return message;
  }
