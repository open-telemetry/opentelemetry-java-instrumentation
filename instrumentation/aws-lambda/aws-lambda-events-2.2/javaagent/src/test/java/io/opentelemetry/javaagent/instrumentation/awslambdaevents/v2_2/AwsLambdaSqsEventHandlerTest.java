...
class AwsLambdaSqsEventHandlerTest extends AbstractAwsLambdaSqsEventHandlerTest {
  ...
  @Test
  void sameNestedEventReusesProcessingSpan() {
    SQSEvent event = newEvent("same");

    new NestedRequestHandler(event).handleRequest(event, newContext());
  ...
  @Test
  void distinctNestedEventStartsProcessingSpan() {
    SQSEvent outerEvent = newEvent("outer");
    SQSEvent nestedEvent = newEvent("nested");

    new NestedRequestHandler(nestedEvent).handleRequest(outerEvent, newContext());
