...
class HbaseServerTargetTest {
  ...
  void rendersCanonicalZooKeeperClusterKey() {
    ...
    assertThat(HbaseServerTarget.from(configuration)).isEqualTo("zk-a,zk-b:2222:3218:/production");
  }
  ...
  void usesDefaultMasterPortWhenConfiguredPortIsZero() {
    ...
    assertThat(HbaseServerTarget.from(configuration)).isEqualTo("master-a:16000");
  }
  ...
  void rejectsIncompleteOrInvalidConfiguration() {
    ...
    assertThat(HbaseServerTarget.from(masterConfiguration)).isNull();
