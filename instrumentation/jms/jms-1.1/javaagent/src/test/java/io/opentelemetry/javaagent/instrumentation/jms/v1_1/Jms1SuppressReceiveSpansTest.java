...
class Jms1SuppressReceiveSpansTest extends AbstractJms1Test {
  ...
  boolean expectReceiveSpan() {
    // legacy receive spans are still emitted; under the stable semantic conventions receive spans
    // are off by default, so a successful receive records metrics only
  ...
  void testMessageConsumer(
    ...
    if (emitStableMessagingSemconv()) {
      // under the stable messaging semantic conventions with receive spans off (the default) a
      // successful application-initiated receive records metrics but produces no receive span
