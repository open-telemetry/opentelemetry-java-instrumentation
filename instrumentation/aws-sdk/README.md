...
| `otel.instrumentation.aws-sdk.batch-send.message-creation-spans.enabled`   | Boolean | `true`  | Emit a create span for each eligible message of an SQS batch send. Overrides the common messaging setting.                            |
| `otel.instrumentation.messaging.batch-send.message-creation-spans.enabled` | Boolean | `true`  | Default for batch-send message creation spans when an instrumentation-specific setting is not configured.                             |
