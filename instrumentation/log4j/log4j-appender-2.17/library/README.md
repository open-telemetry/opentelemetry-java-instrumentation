...
| `captureContextDataAttributes`     | String  |         | Comma separated list of context data attributes to capture. Use the wildcard character `*` to capture all attributes. When using `*`, prefix a key with `!` to exclude it (e.g. `*,!foo` captures all attributes except `foo`). |
