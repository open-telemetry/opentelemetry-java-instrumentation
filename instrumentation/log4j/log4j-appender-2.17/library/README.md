...
| `captureContextDataAttributes`     | String  |         | Comma separated list of context data attributes to capture. Use the wildcard character `*` to capture all attributes; glob wildcards (`*` and `?`) are supported.                                          |
| `excludeContextDataAttributes`     | String  |         | Comma separated list of context data attributes to exclude from capture (glob wildcards `*` and `?` supported). Only takes effect alongside `captureContextDataAttributes`.                                |
