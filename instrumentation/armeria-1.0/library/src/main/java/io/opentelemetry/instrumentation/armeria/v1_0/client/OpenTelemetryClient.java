...
public class OpenTelemetryClient extends SimpleDecoratingHttpClient {
  ...
  public HttpResponse execute(ClientRequestContext ctx, HttpRequest req) throws Exception {
    ...
    HttpClientOperation<RequestLog> operation =
        clientTracer.startOperation(ctx, ctx, requestStartTimeNanos);
          ...
          .thenAccept(
              log -> {
                ...
                if (log.responseCause() != null) {
                  operation.endExceptionally(log, log.responseCause(), requestEndTimeNanos);
                } else {
