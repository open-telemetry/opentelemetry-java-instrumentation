/*
 * Copyright The OpenTelemetry Authors
 * SPDX-License-Identifier: Apache-2.0
 */

package io.opentelemetry.javaagent.instrumentation.opentelemetryapi.v1_65.incubator.metrics;

import io.opentelemetry.api.incubator.metrics.ExtendedLongHistogram;
import io.opentelemetry.api.metrics.LongHistogram;
import io.opentelemetry.javaagent.instrumentation.opentelemetryapi.v1_0.trace.Bridging;
import io.opentelemetry.javaagent.instrumentation.opentelemetryapi.v1_10.metrics.ApplicationLongHistogram;

// extends the plain (non-incubator) bridge directly instead of the 1.40 incubator marker class,
// since the 1.40 incubator marker class does not implement ExtendedLongHistogram#bind(Attributes)
// (added in 1.65) and would otherwise make this class fail muzzle validation
final class ApplicationLongHistogram165Incubator extends ApplicationLongHistogram
    implements application.io.opentelemetry.api.incubator.metrics.ExtendedLongHistogram {

  private final ExtendedLongHistogram agentHistogram;

  ApplicationLongHistogram165Incubator(LongHistogram agentHistogram) {
    super(agentHistogram);
    this.agentHistogram = (ExtendedLongHistogram) agentHistogram;
  }

  @Override
  public application.io.opentelemetry.api.incubator.metrics.BoundLongHistogram bind(
      application.io.opentelemetry.api.common.Attributes applicationAttributes) {
    return new ApplicationBoundLongHistogram165Incubator(
        agentHistogram.bind(Bridging.toAgent(applicationAttributes)));
  }
}
