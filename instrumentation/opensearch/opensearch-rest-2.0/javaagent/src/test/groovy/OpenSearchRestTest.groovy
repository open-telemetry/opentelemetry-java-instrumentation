...
class OpenSearchRestTest extends AgentInstrumentationSpecification {
  ...
  def setupSpec() {
    opensearch = new OpensearchContainer(DockerImageName.parse("opensearchproject/opensearch:2.3.0")).withSecurityEnabled()
    // limit memory usage
      ...
      trace(0, 4) {
        ...
        span(2) {
          ...
          attributes {
            ...
            "$SemanticAttributes.HTTP_URL" "${httpHost.toURI()}/_cluster/health"
            "$SemanticAttributes.HTTP_STATUS_CODE" 200
            "$SemanticAttributes.HTTP_RESPONSE_CONTENT_LENGTH" 449
          }
        }
        span(3) {
          name "callback"
