...
public class PekkoRouteWrapper implements Function1<RequestContext, Future<RouteResult>> {
  ...
  public Future<RouteResult> apply(RequestContext ctx) {
    PekkoRouteHolder parentRouteHolder =
        ctx.request().getAttribute(PekkoRouteHolder.ATTRIBUTE_KEY).orElse(null);
    if (parentRouteHolder == null) {
      ...
      RequestContext requestWithChildRouteHolder =
          ctx.mapRequest(
              req ->
            ...
            .transform(
                tryResult -> {
                  ...
                  if (tryResult.isFailure()) {
                    ...
                    // A parent routeHolder will eventually be added to the httpresponse attribute
                    // after bubbling up the Directive stack to a pekko exception handler, which
                    // converts the error to a successful httpresponse.
