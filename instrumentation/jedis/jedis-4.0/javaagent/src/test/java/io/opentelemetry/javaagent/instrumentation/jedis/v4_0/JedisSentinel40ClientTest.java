...
class JedisSentinel40ClientTest {
  ...
  @Test
  void sentineledConnectionProviderUsesConfiguredTarget() throws Exception {
    assumeTrue(classPresent("redis.clients.jedis.providers.SentineledConnectionProvider"));

    Class<?> providerClass =
        Class.forName("redis.clients.jedis.providers.SentineledConnectionProvider");
    JedisClientConfig clientConfig = DefaultJedisClientConfig.builder().build();
    Object provider =
        providerClass
            .getConstructor(
                String.class, JedisClientConfig.class, Set.class, JedisClientConfig.class)
            .newInstance(
                MASTER_NAME,
                clientConfig,
                singleton(new HostAndPort(sentinelServer.getHost(), sentinelPort)),
                clientConfig);
    cleanup.deferCleanup((AutoCloseable) provider);

    Connection connection = (Connection) providerClass.getMethod("getConnection").invoke(provider);
    try (Jedis jedis = new Jedis(connection)) {
      testing.clearData();
      jedis.set("sentineled-provider", "value");
    }
