...
| `captureMdcAttributes`               | String  |         | Comma separated list of MDC attributes to capture. Use the wildcard character `*` to capture all attributes; glob wildcards (`*` and `?`) are supported.                                                                                          |
| `excludeMdcAttributes`               | String  |         | Comma separated list of MDC attributes to exclude from capture (glob wildcards `*` and `?` supported). Only takes effect alongside `captureMdcAttributes`.                                                                                        |
