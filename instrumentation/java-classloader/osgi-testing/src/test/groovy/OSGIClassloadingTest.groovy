...
class OSGIClassloadingTest extends AgentTestRunner {
    def "OSGI delegates to bootstrap class loader for agent classes"() {
        ...
        def clazz
        if (args == 1) {
            clazz = loader.loadClass("io.opentelemetry.instrumentation.api.MoreAttributes")
        } else {
            clazz = loader.loadClass("io.opentelemetry.instrumentation.api.MoreAttributes", false)
        }
