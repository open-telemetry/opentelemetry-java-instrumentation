...
class VertxRedisClientAttributesGetterTest {
  ...
  @Test
  void uncapturedTargetUsesSelectedEndpoint() {
    VertxRedisClientRequest request = request(null, false);

    assertThat(getter.getServerAddress(request)).isEqualTo(SELECTED_HOST);
    assertThat(getter.getServerPort(request)).isEqualTo(SELECTED_PORT);
    assertThat(request.getPeerAddress()).isEqualTo(PEER_HOST);
    assertThat(request.getPeerPort()).isEqualTo(PEER_PORT);
  }
  ...
  private static VertxRedisClientRequest request(RedisServerTarget target, boolean captured) {
    ...
    return new VertxRedisClientRequest("GET", emptyList(), redisUri, netSocket);
  }
}
