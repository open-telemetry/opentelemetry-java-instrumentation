...
final class McpServerToolCallMono {
  private static final Logger logger = Logger.getLogger(McpServerToolCallMono.class.getName());
        ...
        reactorContext -> {
          try {
            ...
            try {
              ...
              logger.log(FINE, "Failed to wrap MCP server tool call publisher", t);
              endSpan(context, request);
            }
          } catch (Throwable t) {
            logger.log(FINE, "Failed to trace MCP server tool call", t);
  ...
  private static void endSpan(Context context, McpServerToolCallRequest request) {
    try {
      serverInstrumenter().end(context, request, null, null);
    } catch (Throwable t) {
      logger.log(FINE, "Failed to end MCP server tool call span", t);
      // Ignore instrumentation failures to preserve application behavior.
    }
  }
