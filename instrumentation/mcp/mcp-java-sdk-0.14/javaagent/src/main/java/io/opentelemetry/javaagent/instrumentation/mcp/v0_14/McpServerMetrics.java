...
import static io.opentelemetry.semconv.incubating.McpIncubatingMetrics.MCP_SERVER_OPERATION_DURATION_DESCRIPTION;
import static io.opentelemetry.semconv.incubating.McpIncubatingMetrics.MCP_SERVER_OPERATION_DURATION_NAME;
import static io.opentelemetry.semconv.incubating.McpIncubatingMetrics.MCP_SERVER_OPERATION_DURATION_UNIT;
...
final class McpServerMetrics implements OperationListener {
  private static final double NANOS_PER_S = SECONDS.toNanos(1);
  private static final ContextKey<State> STATE_KEY = ContextKey.named("mcp-server-metrics-state");
  private static final Logger logger = Logger.getLogger(McpServerMetrics.class.getName());

  static OperationMetrics get() {
    return OperationMetricsUtil.create("mcp server", McpServerMetrics::new);
  }

  private final DoubleHistogram operationDuration;

  @SuppressWarnings("deprecation") // using deprecated semconv
  private McpServerMetrics(Meter meter) {
    DoubleHistogramBuilder builder =
        meter
            .histogramBuilder(MCP_SERVER_OPERATION_DURATION_NAME)
            .setUnit(MCP_SERVER_OPERATION_DURATION_UNIT)
            .setDescription(MCP_SERVER_OPERATION_DURATION_DESCRIPTION)
  ...
  public void onEnd(Context context, Attributes endAttributes, long endNanos) {
    ...
    if (state == null) {
      logger.log(FINE, "No state present when ending MCP server operation metrics");
      return;
    }
