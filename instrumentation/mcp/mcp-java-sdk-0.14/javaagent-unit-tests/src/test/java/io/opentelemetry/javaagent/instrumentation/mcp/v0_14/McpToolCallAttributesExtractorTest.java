...
class McpToolCallAttributesExtractorTest {
  ...
  void extractsToolCallAttributesWithoutPayloads() {
    McpToolCallRequest request = new McpToolCallRequest("delete-file", "2024-11-05");
    ...
    assertThat(attributes.build().asMap())
        ...
        .containsEntry(MCP_PROTOCOL_VERSION, "2024-11-05")
  ...
  void extractsJsonRpcErrorCode() {
    McpToolCallRequest request = new McpToolCallRequest("lookup", null);
    AttributesBuilder attributes = Attributes.builder();
    McpError error =
  ...
  @Test
  void handlesJsonRpcErrorWithoutCode() {
    McpToolCallRequest request = new McpToolCallRequest("lookup", null);
    AttributesBuilder attributes = Attributes.builder();
    McpError error =
        new McpError(new McpSchema.JSONRPCResponse.JSONRPCError(null, "Unknown error", null));

    extractor.onEnd(attributes, Context.root(), request, null, error);

    assertThat(attributes.build().asMap())
        .containsEntry(ERROR_TYPE, McpError.class.getName())
