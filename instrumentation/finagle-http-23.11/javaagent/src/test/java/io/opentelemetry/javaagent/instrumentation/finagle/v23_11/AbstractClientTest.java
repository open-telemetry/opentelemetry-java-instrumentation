...
abstract class AbstractClientTest {
  ...
  protected abstract Http.Client createClient(ClientType clientType);
  ...
  Service<Request, Response> getClient(URI uri) {
    return getClient(uri, uri.getScheme().equals("https") ? ClientType.TLS : ClientType.DEFAULT);
  }

  Service<Request, Response> getClient(URI uri, ClientType clientType) {
    return clients.computeIfAbsent(
        clientType,
        (type) -> createClient(type).newService(uri.getHost() + ":" + Utils.safePort(uri)));
  }

  Future<Response> doSendRequest(Request request, URI uri) {
  ...
  @Nested
  class FinagleClientTest extends AbstractHttpClientTest<Request> {
