...
import io.opentelemetry.instrumentation.sofarpc.v5_4.impl.MiddleServiceImpl;
...
public abstract class AbstractSofaRpcTraceChainTest {
  ...
  ConsumerConfig<GenericService> configureGenericMiddleClient(int port) {
    ...
    consumer
        .setInterfaceId(MiddleService.class.getName())
        .setApplication(new ApplicationConfig().setAppName("sofa-rpc-test-consumer"))
        .setDirectUrl("bolt://127.0.0.1:" + port)
        .setRegister(false)
        .setTimeout(30000)
  ...
  ProviderConfig<MiddleService> configureMiddleServer(
      int port, ConsumerConfig<HelloService> consumerConfig) {
  ...
  void testSofaRpcChain() {
    ...
    // set hello service consumer
    ConsumerConfig<HelloService> helloConsumerConfig = configureClient(port);
    cleanup.deferCleanup(helloConsumerConfig::unRefer);

    // setup middle service provider
    ProviderConfig<MiddleService> middleProviderConfig =
        configureMiddleServer(middlePort, helloConsumerConfig);
                        ...
                        span.hasName(
                            ...
                            .hasAttributesSatisfyingExactly(
                                ...
                                equalTo(
                                    RPC_SERVICE,
                                    emitOldRpcSemconv()
                                        ? "io.opentelemetry.instrumentation.sofarpc.v5_4.api.MiddleService"
                                        : null),
                                equalTo(
                                    RPC_METHOD,
                                    emitStableRpcSemconv()
  ...
  void testSofaRpcChainLocal() {
    ...
    ProviderConfig<MiddleService> middleProviderConfig =
        configureMiddleServer(port, helloConsumerConfig);
    cleanup.deferCleanup(middleProviderConfig::unExport);
    middleProviderConfig.export();
                        ...
                        span.hasName(
                            ...
                            .hasAttributesSatisfyingExactly(
                                ...
                                satisfies(
                                    NETWORK_PEER_PORT,
                                    AbstractSofaRpcTest::assertNetworkPeerPort))),
            // Second trace: HelloService SERVER span (appears in separate trace because CLIENT span
            // was skipped)
            // This is expected behavior when CLIENT span is skipped - trace context may not
            // propagate
            trace ->
                trace.hasSpansSatisfyingExactly(
                    span ->
                                          ...
                                          point ->
                                              point.hasAttributesSatisfyingExactly(
                                                  ...
                                                  equalTo(
                                                      ...
                                                      "io.opentelemetry.instrumentation.sofarpc.v5_4.api.MiddleService"),
                                                  equalTo(RPC_METHOD, "hello")),
                                          // HelloService SERVER metrics are also kept for local
                                          // calls
                                          point ->
                                              point.hasAttributesSatisfyingExactly(
                                                  equalTo(RPC_SYSTEM, "sofa_rpc"),
