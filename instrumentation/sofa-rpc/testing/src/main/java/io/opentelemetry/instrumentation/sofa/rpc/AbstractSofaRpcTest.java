...
import io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService;
import io.opentelemetry.instrumentation.sofa.rpc.api.HelloService;
import io.opentelemetry.instrumentation.sofa.rpc.impl.ErrorServiceImpl;
...
public abstract class AbstractSofaRpcTest {
  ...
  @BeforeAll
  static void setUp() {
    // SOFARPC doesn't need special setup
  }
  ...
  ProviderConfig<ErrorService> configureErrorServer(int port) {
    ...
    provider.setInterfaceId(ErrorService.class.getName())
  ...
  ConsumerConfig<ErrorService> configureErrorClient(int port) {
    ...
    consumer.setInterfaceId(ErrorService.class.getName())
  ...
  void testSofaRpcBase() {
    ...
    // Start consumer with generic service
  ...
  void testSofaRpcAsync() throws InterruptedException {
    ...
    // Start consumer with async invoke type
    ConsumerConfig<GenericService> consumerConfig = configureGenericClient(port);
    ...
    Object result =
        runWithSpan(
            "parent",
            () -> {
              // $invoke returns null for future calls, need to get Future from context
              genericService.$invoke(
                  "hello", new String[] {String.class.getName()}, new Object[] {"hello"});
              // Get Future from SofaResponseFuture
  ...
  void testSofaRpcRpcException() {
        ...
        .waitAndAssertTraces(
            trace ->
                trace.hasSpansSatisfyingExactly(
                    ...
                    span ->
                        span.hasName(
                                "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService/throwException")
                            .hasKind(SpanKind.CLIENT)
                            .hasParent(trace.getSpan(0))
                            ...
                            .hasAttributesSatisfyingExactly(
                                ...
                                equalTo(
                                    RPC_SERVICE,
                                    "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService"),
                                ...
                                satisfies(
                                    ...
                                    errorType ->
                                        errorType
                                            ...
                                            .satisfies(
                                                str ->
                                                    assertThat(str).contains("SofaRpcRuntimeException")))),
                    span ->
                        span.hasName(
                                "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService/throwException")
                            .hasKind(SpanKind.SERVER)
    ...
    // Verify metrics
    // Note: error.type is recorded in span attributes, not in metrics point attributes
    // (it may appear in exemplars, but we don't assert on exemplars here)
    testing()
        .waitAndAssertMetrics(
            "io.opentelemetry.sofa-rpc",
            "rpc.server.duration",
        ...
        .waitAndAssertTraces(
            trace ->
                trace.hasSpansSatisfyingExactly(
                    ...
                    span ->
                        span.hasName(
                                "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService/throwBusinessException")
                            .hasKind(SpanKind.CLIENT)
                            .hasParent(trace.getSpan(0))
                            .hasStatus(StatusData.error())
                            .hasAttributesSatisfyingExactly(
                                ...
                                satisfies(
                                    ...
                                    errorType ->
                                        errorType
                                            ...
                                            .satisfies(
                                                str ->
                                                    assertThat(str).contains("IllegalStateException")))),
                    span ->
                        span.hasName(
                                "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService/throwBusinessException")
                            .hasKind(SpanKind.SERVER)
  ...
  void testSofaRpcTimeout() {
    ...
    // Timeout is a client-side behavior, but the server may have already received and started
    // processing the request, so we may see both CLIENT and SERVER spans. We use hasSpansSatisfying
    // instead of hasSpansSatisfyingExactly to allow the optional SERVER span.
    testing()
        .waitAndAssertTraces(
            trace ->
                trace.hasSpansSatisfyingExactly(
                    ...
                    span ->
                        span.hasName(
                                "io.opentelemetry.instrumentation.sofa.rpc.api.ErrorService/timeout")
                            .hasKind(SpanKind.CLIENT)
                            .hasParent(trace.getSpan(0))
                            .hasStatus(StatusData.error())
                            .hasAttributesSatisfyingExactly(
                                                ...
                                                str ->
                                                    assertThat(str)
                                                        .satisfiesAnyOf(
                                                            ...
                                                            s -> assertThat(s).contains("Timeout"),
                                                            s -> assertThat(s).contains("SofaTimeOutException")))))));
  }
}
