...
dependencies {
  implementation(project(":instrumentation:sofa-rpc:library-autoconfigure"))
...
testing {
  suites {
    // using a test suite to ensure that project(":instrumentation:sofa-rpc:library-autoconfigure")
    ...
    val testSofaRpc by registering(JvmTestSuite::class) {
      dependencies {
        implementation(project(":instrumentation:sofa-rpc:testing"))
        if (latestDepTest) {
