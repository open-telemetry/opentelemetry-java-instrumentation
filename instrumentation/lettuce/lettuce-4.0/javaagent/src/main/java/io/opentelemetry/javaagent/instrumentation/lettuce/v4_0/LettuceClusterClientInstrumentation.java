...
class LettuceClusterClientInstrumentation implements TypeInstrumentation {
  ...
  public void transform(TypeTransformer transformer) {
    ...
    // covers node connections in lettuce 4.0-4.3, which connect through connectToNode
    transformer.applyAdviceToMethod(
        named("connectToNode")
        ...
        getClass().getName() + "$AttachConnectionAdvice");
    // covers every connection in lettuce 4.2+, including the node connections that lettuce 4.4+
    // opens through connectToNodeAsync instead of connectToNode
    transformer.applyAdviceToMethod(
  ...
  public static class AttachStatefulConnectionAdvice {
    ...
    // runs before lettuce dispatches the connection initialization commands
