...
public class LettuceMonoDualConsumer<R, T> implements Consumer<R>, BiConsumer<T, Throwable> {
   ...
   * doOnSuccessOrError} was removed in reactor 3.5, so {@code doOnSuccess} + {@code doOnError} (both
   * available across the whole supported reactor range) are used instead. The wiring lives here, on
   * an injected helper class, rather than inline in the advice, so the lambdas do not become private
   * synthetic methods on the advice class (which the instrumented class cannot access).
   */
