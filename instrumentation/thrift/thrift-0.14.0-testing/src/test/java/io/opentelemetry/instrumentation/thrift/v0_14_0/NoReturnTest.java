...
public class NoReturnTest extends ThriftBaseTest {
  ...
  @Test
  public void syncNonblockingSaslClientSyncSaslNonblockingServerNoReturnError() {
    Exception error = null;
    try {
      this.startSyncSaslNonblockingServer(this.port);
      this.syncNonblockingSaslClientNoReturn(this.port);
    } catch (IOException | TException e) {
      error = e;
    }
    Assertions.assertThat(error).isNull();
    this.waitAndAssertTracesClientSyncNoServerNoSasl("noReturn", 1);
  }

  @Test
  public void syncNonblockingSaslClientAsyncSaslNonblockingServerNoReturnError() {
    Exception error = null;
    try {
  ...
  public void syncFramedClientNoReturn(int port) throws TException {
    try (TSocket tSocket = new TSocket("localhost", port)) {
