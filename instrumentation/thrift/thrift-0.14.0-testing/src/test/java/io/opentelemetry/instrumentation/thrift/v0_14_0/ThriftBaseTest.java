...
import io.opentelemetry.instrumentation.thrift.v0_14_0.server.ThriftServiceAsyncImpl;
import io.opentelemetry.instrumentation.thrift.v0_14_0.server.ThriftServiceImpl;
...
public abstract class ThriftBaseTest {
  ...
  private static final String ASYNC_CLIENT =
      "io.opentelemetry.instrumentation.thrift.v0_14_0.thrift.ThriftService$AsyncClient";
  private static final String SYNC_CLIENT =
      "io.opentelemetry.instrumentation.thrift.v0_14_0.thrift.ThriftService$Client";
  private static final String ASYNC_SERVER =
      "io.opentelemetry.instrumentation.thrift.v0_14_0.server.ThriftServiceAsyncImpl";
  private static final String SYNC_SERVER =
      "io.opentelemetry.instrumentation.thrift.v0_14_0.server.ThriftServiceImpl";
  ...
  public void startAsyncSimpleServer(int port) throws TTransportException {
    ...
    new Thread(
            () -> {
              logger.info(
                  "Starting startAsyncSimpleServer threadName="
                      + Thread.currentThread().getName()
                      + ", threadId="
                      + Thread.currentThread().getId());
              this.server.serve();
  ...
  public void startAsyncThreadPoolServer(int port) throws TTransportException {
    ThriftServiceAsyncImpl impl = new ThriftServiceAsyncImpl();
    ThriftService.AsyncProcessor<ThriftService.AsyncIface> processor =
        new ThriftService.AsyncProcessor<ThriftService.AsyncIface>(impl);
  ...
  public void startAsyncSaslNonblockingServer(int port) throws TTransportException, IOException {
    ThriftServiceAsyncImpl impl = new ThriftServiceAsyncImpl();
    ThriftService.AsyncProcessor<ThriftService.AsyncIface> processor =
        new ThriftService.AsyncProcessor<ThriftService.AsyncIface>(impl);
    TNonblockingServerTransport transport = new TNonblockingServerSocket(port);
    Map<String, String> saslOptions = new HashMap<>();
    saslOptions.put("javax.security.sasl.qop", "auth");
    saslOptions.put("javax.security.sasl.server.authentication", "true");
    TSaslServerTransport.Factory saslTransportFactory = new TSaslServerTransport.Factory();
    saslTransportFactory.addServerDefinition(
        "PLAIN", null, null, saslOptions, new TestSaslCallbackHandler("12345"));
    TBinaryProtocol.Factory protocolFactory = new TBinaryProtocol.Factory();
    TSaslNonblockingServer.Args serverArgs =
        new TSaslNonblockingServer.Args(transport)
            .processor(processor)
            .protocolFactory(protocolFactory)
            .transportFactory(saslTransportFactory);
    this.server = new TSaslNonblockingServer(serverArgs);
    new Thread(
            () -> {
              logger.info(
                  "Starting startAsyncSaslNonblockingServer threadName="
                      + Thread.currentThread().getName()
                      + ", threadId="
                      + Thread.currentThread().getId());
              this.server.serve();
  ...
  public void startSyncSaslNonblockingServer(int port) throws TTransportException, IOException {
    ThriftServiceImpl impl = new ThriftServiceImpl();
    ThriftService.Processor<ThriftService.Iface> processor =
        new ThriftService.Processor<ThriftService.Iface>(impl);
    TNonblockingServerTransport transport = new TNonblockingServerSocket(port);
  ...
  public void waitAndAssertTracesClientSyncServerSyncWithError(String method, int count) {
    this.baseWaitAndAssertTraces(
        ...
        PEER_NAME,
        PEER_ADDR,
        val -> val.isIn("Internal error processing " + method),
        APP_EXCEPTION,
        SYNC_SERVER,
        StatusData.unset(),
        null,
  ...
  public void waitAndAssertTracesClientSyncServerSyncWithError(
    ...
    this.baseWaitAndAssertTraces(
        ...
        PEER_NAME,
        PEER_ADDR,
        val -> val.isIn("Internal error processing " + clientMethod),
        APP_EXCEPTION,
        SYNC_SERVER,
        StatusData.unset(),
        null,
  ...
  public void waitAndAssertTracesClientSyncNoServer(String method, int count) {
    this.baseWaitAndAssertTraces(
        ...
        PEER_NAME,
        PEER_ADDR,
        val -> val.isIn("Socket is closed by peer.", "java.net.SocketException: Connection reset"),
        TRANSPORT_EXCEPTION,
        null,
        null,
        null,
