...
public class WithStructTest extends ThriftBaseTest {
  ...
  @Test
  public void syncNonblockingSaslClientSyncSaslNonblockingServerWithStructError() {
    Exception error = null;
    try {
      this.startSyncSaslNonblockingServer(this.port);
      this.syncNonblockingSaslClientWithStruct(this.port);
    } catch (IOException | TException e) {
      error = e;
    }
    Assertions.assertThat(error).isNotNull();
    this.waitAndAssertTracesClientSyncNoServerNoSasl("data", 1);
  }

  @Test
  public void syncNonblockingSaslClientAsyncSaslNonblockingServerWithStructError() {
    Exception error = null;
    try {
  ...
  public void nonBlockClientWithStruct(int port) throws TException, IOException {
    ...
    User user = new User("Bob", "1", 20);
    Account account = new Account("US", "123456");
    AsyncMethodCallback<UserAccount> callback =
        new AsyncMethodCallback<UserAccount>() {
          @Override
          public void onComplete(UserAccount response) {
            Assertions.assertThat(response.user).isEqualTo(user);
            Assertions.assertThat(response.account).isEqualTo(account);
          }
