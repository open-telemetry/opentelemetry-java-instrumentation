...
public class WithErrorTest extends ThriftBaseTest {
  ...
  @Test
  public void syncNonblockingSaslClientSyncSaslNonblockingServerWithErrorError() {
    Exception error = null;
    try {
      this.startSyncSaslNonblockingServer(this.port);
      this.syncNonblockingSaslClientWithError(this.port);
    } catch (IOException | TException e) {
      error = e;
    }
    Assertions.assertThat(error).isNull();
    this.waitAndAssertTracesClientSyncNoServerNoSasl("withError", 1);
  }

  @Test
  public void syncNonblockingSaslClientAsyncSaslNonblockingServerWithErrorError() {
    Exception error = null;
    try {
    ...
    AsyncMethodCallback<String> callback =
        new AsyncMethodCallback<String>() {
          ...
          public void onComplete(String s) {
            Assertions.assertThat(s).isEqualTo("Hello USs' Bob");
          }
