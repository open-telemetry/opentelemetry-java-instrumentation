...
public class OneWayErrorTest extends ThriftBaseTest {
  ...
  @Test
  public void nonBlockMultiClientSyncHsHaServerMultiOneWayWithError()
      throws TException, IOException {
    this.startMultiSyncHsHaServer(this.port);
    this.nonBlockClientMultiOneWayWithError(this.port);
    this.waitAndAssertTracesClientAsyncServerSyncOnewayError(
        "oneWayWithError", "syncHelloWorld:oneWayWithError", 1);
  }
  ...
  public void nonBlockClientMultiOneWayWithError(int port) throws TException, IOException {
    TNonblockingTransport transport = new TNonblockingSocket("localhost", port);
    TAsyncClientManager clientManager = new TAsyncClientManager();
    TProtocolFactory protocolFactory = new TBinaryProtocol.Factory();
