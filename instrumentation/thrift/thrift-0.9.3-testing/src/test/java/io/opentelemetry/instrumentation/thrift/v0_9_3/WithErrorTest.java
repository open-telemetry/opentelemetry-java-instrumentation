...
public class WithErrorTest extends ThriftBaseTest {
  ...
  @Test
  public void nonBlockMultiClientSyncHsHaServerMultiWithError() throws TException, IOException {
    this.startMultiSyncHsHaServer(this.port);
    this.nonBlockClientMultiWithError(this.port);
    this.waitAndAssertTracesClientAsyncServerSyncWithError(
        "withError", "syncHelloWorld:withError", 1);
  }
  ...
  public void nonBlockClientMultiWithError(int port) throws TException, IOException {
    TNonblockingTransport transport = new TNonblockingSocket("localhost", port);
    TAsyncClientManager clientManager = new TAsyncClientManager();
    TProtocolFactory protocolFactory = new TBinaryProtocol.Factory();
