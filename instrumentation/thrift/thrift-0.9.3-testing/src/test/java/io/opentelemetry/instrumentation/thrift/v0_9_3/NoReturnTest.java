...
public class NoReturnTest extends ThriftBaseTest {
  ...
  @Test
  public void nonBlockMultiClientSyncHsHaServerMultiNoReturn() throws TException, IOException {
    this.startMultiSyncHsHaServer(this.port);
    this.nonBlockClientMultiNoReturn(this.port);
    this.waitAndAssertTracesClientAsyncServerSync("noReturn", "syncHelloWorld:noReturn", 1);
  }
  ...
  public void nonBlockClientMultiNoReturn(int port) throws TException, IOException {
    TNonblockingTransport transport = new TNonblockingSocket("localhost", port);
    TAsyncClientManager clientManager = new TAsyncClientManager();
    TProtocolFactory protocolFactory = new TBinaryProtocol.Factory();
