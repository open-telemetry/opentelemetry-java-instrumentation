...
class SqlQuerySummaryEdgeCasesTest {
  ...
  private static SqlQuery sanitize(String sql) {
    return SqlQuerySanitizer.create(true).sanitizeWithSummary(sql);
  }
  ...
  private static Stream<Arguments> valuesArgs() {
    return Stream.of(
        ...
        // VALUES in FROM clause — no table name to extract, bare "SELECT" is correct
  ...
  private static Stream<Arguments> aliasEdgeCasesArgs() {
    return Stream.of(
        // Alias that is a keyword - works for "join"
  ...
  private static Stream<Arguments> lexicalEdgeCasesArgs() {
    return Stream.of(
        // Block comments containing keywords
