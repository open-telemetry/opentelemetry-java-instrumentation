...
final class RpcMetricsAdvice {
  ...
  private static final List<AttributeKey<?>> attributesAdvice = new ArrayList<>();

  static {
    attributesAdvice.addAll(
        Arrays.asList(
    ...
    if (SemconvStability.emitOldDatabaseSemconv()) {
      attributesAdvice.add(NetworkAttributes.NETWORK_TYPE);
      attributesAdvice.add(NetworkAttributes.NETWORK_TRANSPORT);
    }
  }
  ...
  static void applyClientDurationAdvice(DoubleHistogramBuilder builder) {
    ...
    ((ExtendedDoubleHistogramBuilder) builder).setAttributesAdvice(attributesAdvice);
  }
