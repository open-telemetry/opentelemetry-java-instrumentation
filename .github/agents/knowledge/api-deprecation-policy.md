...
#### Also set the instrumentation version explicitly
...
name passed in — so when the default scope name is the **old** one, the lookup returns
`null`, the emitted scope has no version, and `TelemetryDataUtil.assertScopeVersion` fails
every test in CI with:
