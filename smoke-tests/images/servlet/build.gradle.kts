  ...
  mapOf(
    ...
    "liberty" to
      listOf(
        ImageTarget(
          listOf(
            "open-liberty:20.0.0.12-full-java11-openj9@sha256:2fa4af95d6c48e3db79edfd2b8a9c71e26c63a68c3fcae92f222fbb42c469ed2",
          ),
          listOf("hotspot", "openj9"),
          listOf("8", "11"),
        ),
        ImageTarget(
          listOf(
            "open-liberty:21.0.0.12-full-java11-openj9@sha256:eb014c600b5e08b799cb0c5781e606cf1e7a28ad913ba956c9d9e7f8a2f528dc",
          ),
          listOf("hotspot", "openj9"),
          listOf("8", "11", "17"),
        ),
        ImageTarget(
          listOf(
            "open-liberty:22.0.0.12-full-java11-openj9@sha256:a06f1da35a564f00354b86c7d01d8cc9d6eef156ce88d5b59605c5c02bf48c72",
          ),
          listOf("hotspot", "openj9"),
          listOf("8", "11", "17"),
        ),
        ImageTarget(
          listOf(
            "open-liberty:23.0.0.12-full-java11-openj9@sha256:cd6aa69cffffb45427cbb6a5640cd00b13c98064f296a66894ea1decd181e1c3",
          ),
          listOf("hotspot", "openj9"),
          listOf("8", "11", "17", "21"),
        ),
    ...
    "payara" to
      listOf(
        ...
        ImageTarget(
          listOf(
            "payara/server-full:6.2023.12@sha256:5c382db1f5bad8bef693dbcd0fed299844690839f4894e07c248de5f4b186b9b",
          ),
          listOf("hotspot"),
          listOf("11", "17"),
          war = "servlet-5.0",
        ...
        for (entry in matrices) {
          for (version in entry.version) {
            ...
            for (jdk in entry.jdk) {
              for (vm in entry.vm) {
                println(
                  "@AppServer(version = \"$version\", jdk = \"$jdk${if (vm == "hotspot") "" else "-openj9"}\")",
                )
                println(
                  "class ${serverName}${majorVersion}Jdk${jdk}${if (vm == "hotspot") "" else "Openj9"} extends ${serverName}SmokeTest {",
                )
                println("}")
              }
            }
...
fun configureImage(
  ...
  val dockerWorkingDir =
    layout.buildDirectory.dir(
      "docker-$server-$version-jdk$jdk-$vm-$warProject-$platformSuffix",
    )

  val prepareTask =
    tasks.register<Copy>("${server}ImagePrepare-$version-jdk$jdk-$vm$platformSuffix") {
  ...
  if (server == "wildfly") {
    ...
    extraArgs["baseDownloadUrl"] =
      "https://repo1.maven.org/maven2/org/wildfly/wildfly-dist/$version/wildfly-dist-$version"
  ...
  val buildTask =
    tasks.register<DockerBuildImage>("${server}Image-$version-jdk$jdk$vmSuffix$platformSuffix") {
      dependsOn(prepareTask)
      group = "build"
      description =
        "Builds Docker image with $server $version on JDK $jdk-$vm${if (isWindows) " on Windows" else ""}"
      ...
      buildArgs.set(
        extraArgs +
          mapOf(
            ...
            "vm" to vm,
            "version" to version,
            "jdkImageName" to jdkImageName,
            "jdkImageHash" to jdkImageHash,
            "imageHash" to serverImageHash,
          ),
      )
    }
