...
public abstract class BaseSpanListener implements SpanListener {
  ...
  protected abstract Span.Kind getSpanType();
  ...
  public static void addThrowable(final Span span, final Throwable throwable) {
    ...
    if (message != null) {
      span.setAttribute(SemanticConventions.ERROR_MESSAGE, message);
    }
    span.setAttribute(SemanticConventions.ERROR_KIND, throwable.getClass().getName());

    final StringWriter errorString = new StringWriter();
    throwable.printStackTrace(new PrintWriter(errorString));
    span.setAttribute(SemanticConventions.ERROR_STACK, errorString.toString());
  }
