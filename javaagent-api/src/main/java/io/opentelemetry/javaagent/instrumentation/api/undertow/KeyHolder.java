...
/**
 * Undertow's {@code io.undertow.server.HttpServerExchange} uses {@code io.undertow.util.AttachmentKey} as a key for storing arbitrary
 * data. It uses {@link IdentityHashMap} and thus all keys are compared for equality by object
 * reference. This means that we cannot hold an instance of {@code io.undertow.util.AttachmentKey} in a static field
 ...
 * have a separate value in a static field and {@code io.undertow.server.HttpServerExchange} will treat them as
 * different keys then.
 *
 ...
 * the loading of {@code io.undertow.util.AttachmentKey} class. Class has to be loaded by <i>any</i> classloader that
